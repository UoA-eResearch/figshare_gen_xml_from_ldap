...
def get_user_attributies(ldap:, upi:, attributes:)
  ...
  ldap.search( base: treebase, filter: filter, attributes: %w[cn sn givenname mail employeenumber] ) do |entry|
    attributes.each do |attribute, value|
      response[value] = if value == :email # Horrible hack, as we are still getting the odd user_gal email address.
                          entry[attribute][0].to_s.strip.gsub(/_gal@/, '@')
                        else
                          entry[attribute][0].to_s.strip
                        end
    end
