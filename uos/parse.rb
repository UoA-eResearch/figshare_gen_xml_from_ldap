...
File.open(@uos_filename['uos_file'], 'r') do |fd|
  ...
  fd.each_line(sep => "\r") do |l|
    ...
    node, _descr, uoa_descr, parent = l.split("\t")
...
File.open(OUTPUT_FILE, 'w+') do |fd|
  ...
  @flat_uos.sort.each do |node, attributes|
    if attributes[:faculty].nil?
      fd.puts "  \"#{format('%-15s', (node + '":'))} { \"faculty\": #{format('%12s', 'null')}, \"descr\":   \"#{attributes[:descr]}\" },"
    else
      fd.puts "  \"#{format('%-15s', (node + '":'))} { \"faculty\": #{format('%12s', ('"' + attributes[:faculty] + '"'))}, \"descr\":   \"#{attributes[:descr]}\" },"
    end
  end
  # Couple in the LDAP, that don't look to be in the UoS
  %w[MAIDMENT OUTREACH].each do |s|
    fd.puts "  \"#{format('%-15s', "#{s}\":")} { \"faculty\": #{format('%12s', 'null')}, \"descr\":   \"#{s}\" }," if @flat_uos[s].nil?
  end
  fd.puts "  \"#{format('%-15s', 'EOT":')} { \"faculty\": #{format('%12s', 'null')}, \"descr\":   \"End of Text\" }"
  fd.puts '}'
end
